...
if (!message.member.roles.cache.has(modConfig.penal.banHammer) && !message.member.hasPermission("ADMINISTRATOR")) return message.react(emojis.hata);
if (!member) return main.embedHata(message.author, message.channel, `Komutu doğru kullanmalısın. \`Örnek: ${conf.prefix || '.'}ban @Richârd/ID Sebep\``).sil(10000);
if (!sebep) return main.embedHata(message.author, message.channel, `Komutu doğru kullanmalısın. \`Örnek: ${conf.prefix || '.'}ban @Richârd/ID Sebep\``).sil(10000);
...
let cezaID = penals.cek(`cezanumarasi.${message.guild.id}`)+1;
      ...
      main.embedBasari(message.author, message.channel, `${member} kullanıcısı ${sebep} sebebi ile uzaklaştırıldı! (\`#${cezaID}\`)`)
...
modMain.cezaEkle(cezaID, member.id, member.displayName, message.member.displayName, message.author.id, "BAN", sebep, "KALICI", muteAtılma, "KALICI", "ban")
