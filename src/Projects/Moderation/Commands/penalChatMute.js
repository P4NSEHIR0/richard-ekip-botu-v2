...
module.exports.execute = async(ModerationClient , message, args) => {
  ...
  if (!message.member.roles.cache.has(modConfig.penal.chatmuteHammer) && !message.member.hasPermission("ADMINISTRATOR")) return message.react(emojis.hata);
  if (!member) return main.embedHata(message.author, message.channel, `Komutu doğru kullanmalısın. \`Örnek: ${settings.prefix || '.'}chatmute @Richârd/ID Sebep Süre\``).sil(10000);
  if (!sebep) return main.embedHata(message.author, message.channel, `Komutu doğru kullanmalısın. \`Örnek: ${settings.prefix || '.'}chatmute @Richârd/ID Sebep Süre\``).sil(10000);
  if (!zaman) return main.embedHata(message.author, message.channel, `Komutu doğru kullanmalısın. \`Örnek: ${settings.prefix || '.'}chatmute @Richârd/ID Sebep Süre\``).sil(10000);
...
//////////
    ...
    let zamanimiz = zamanimizlar.replace("second"," saniye").replace("minute"," dakika").replace("hours"," saat").replace("day"," gün")
      ...
      main.embedBasari(message.author, message.channel, `${member} kullanıcısı yazılı kanallarda **${zamanimiz}** susturuldu (\`#${cezaID}\`)`)
    
//////////
modMain.cezaEkle(cezaID, member.id, member.displayName, message.member.displayName, message.author.id, "CHAT-MUTE", sebep, zamanimiz, muteAtılma, muteBitiş, "chatmute")
let ceza = {  
...
member.roles.add(modConfig.penal.chatmuted)
...
setTimeout(() => {
    ...
    member.roles.remove(modConfig.penal.chatmuted)
